...
impl Archive {
    ...
    pub fn load_tables(&mut self) -> Result<()> {
            ...
            && hi_block_pos != 0
        {
            ...
            // FIX : Each entries are u16 (2 bytes) not 8 !
            let hi_block_end = hi_block_offset + (self.header.block_table_size as u64 * 2);
            let file_size = self.reader.get_ref().metadata()?.len();
            // FIX : Reverse the logic, parse if it's less than or equal
