...
pub enum Error {
    ...
    #[error("Header sizes for mipmap {0} are {1}x{2}, but there are {3} pixels actually in content.")]
    MismatchSizes(usize, u32, u32, usize),
    /// Mismatch between expected and actual alpha channel data size
    #[error("Header sizes for mipmap {0} are {1}x{2}, but there are {3} alpha values actually in content.")]
